...
#define DEFAULT_GRASP_STATE_BASE_TOPIC "grasp_state"
...
void GazeboGraspFix::Init()
{
    ...
    if (!ros::isInitialized()) {
      ROS_ERROR_STREAM(handleName << ": ROS has not been initialized.");
      return;
    }
}
...
void GazeboGraspFix::Load(physics::ModelPtr _parent, sdf::ElementPtr _sdf)
{
    ...
    sdf::ElementPtr graspStateBaseTopicElem = _sdf->GetElement("grasp_state_base_topic");
    if (!graspStateBaseTopicElem){
        std::cout<<"GazeboGraspFix: Using default base topic of "<<DEFAULT_GRASP_STATE_BASE_TOPIC<<" because no <grasp_state_base_topic> element specified."<<std::endl;
        this->graspStateBaseTopic = DEFAULT_GRASP_STATE_BASE_TOPIC;
    } else {
        this->graspStateBaseTopic = graspStateBaseTopicElem->Get<std::string>();
    }
    nh = ros::NodeHandle(graspStateBaseTopic);
    ...
    // add all arms:
    for (; armElem != nullptr; armElem = armElem->GetNextElement("arm"))
    {
        ...
        for (; fingerLinkElem != nullptr; fingerLinkElem = fingerLinkElem->GetNextElement("gripper_link"))
        {
        ...
        if (!gripper.Init(_parent, armName, palmName, fingerLinkNames, disableCollisionsOnAttach, _collisions, nh))
        {
