...
bool GazeboGraspGripper::Init(physics::ModelPtr& _model,
    ...
    std::map<std::string, physics::CollisionPtr>& _collisionElems, const ros::NodeHandle& _nh)
{
    ...
    graspStatePub = nh.advertise<std_msgs::Bool>(gripperName + "/object_grasped", 10, true);
    publishGraspState();
...
void GazeboGraspGripper::publishGraspState()
{
  std_msgs::Bool msg;
  msg.data = isObjectAttached();
  graspStatePub.publish(msg);
}
